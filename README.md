...
残りの、できるだけ異なるペアの社員については、異なる日に同じ組合せがあったら駄目にしちゃえばオッケー。簡単にするためにもう少し細かく考えて、たとえば「社員0と社員1で、0日目と1日目が同じならペナルティを与える」とする場合は、以下のコードで表現できます。

~~~ python
h += xs[0][0] * xs[1][0] * xs[0][1] * xs[1][1]
~~~

QUBOだからxsの要素の値は1か0のどちらかなので、だから0日目と1日目の両方に社員0と社員1の両方が出社する場合、つまり全部1の場合以外は0になります。全部1ということは同じ組合せということになるので、その場合に1になるのであればペナルティとして実に都合がよい。あとは、他の社員の組合せと他の日の組合せを網羅するために、4重のループを書いたというわけですな。
