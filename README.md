...
これで、bさんとcさんが和解する等して関係性が変わったとしても、同じコードで表現できるようになりました（実際には、さらに汎用的にするために（`a`や`b`や`c`が1なのか-1なのかを判断するために）もう一つの変数（イジング模型を使用した焼きなまし法のコードでの`hs`）も使用します）。で、この汎用的で素晴らしいこれが、イジング模型なんです。
