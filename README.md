...
あと、大抵の物事には優先順位があり、そして、異なる物事を足し合わせる（今回の適当に作った2つの式の単位系が同じとは思えない）のは困難です。なので、適当な係数、`a`と`b`を掛けることにして、これらの値はチューニングの際に指定できるようにしましょう。それが`Placeholder`です。今回は、いろいろ試してみて`a=1.0`と`b=0.5`にしてみました。と、こんな感じでPyQUBOでQUBOを定義すると、あとは`model.to_ising()`でイジング模型に一発で変換されるというわけ。これをnealで解けば答えが返ってくる。いやぁ、世の中便利になりましたな。
